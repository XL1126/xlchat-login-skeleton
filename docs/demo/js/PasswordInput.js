...
(function (global) {
  ...
  /** props: value, onChange, placeholder, show, field */
  function PasswordInput(props) {
    var field = props.field || 'password';
    var show = !!props.show;
    return (
      '<div class="form-group">' +
      '<div class="password-input-wrapper ' + (props.className || '') + '">' +
      '<div class="ds-form-item__content">' +
      '<div class="ds-input ds-input--none ds-input--bordered ds-input--l">' +
      '<input type="' + (show ? 'text' : 'password') + '" class="ds-input__input" data-field="' + field + '" placeholder="' +
      global.XL.esc(props.placeholder || '') +
      '" value="' +
      global.XL.esc(props.value || '') +
      '" autocomplete="off" autocapitalize="off" spellcheck="false" />' +
      '<div class="ds-input__suffix">' +
      '<button type="button" class="password-toggle" data-action="toggle-pw" data-target="' + field + '" aria-label="' +
      (show ? '隐藏密码' : '显示密码') +
      '">' +
  ...
  /** 只更新本地 DOM，不整页 re-render（对应 PasswordInput 本地 state） */
  PasswordInput.toggle = function (root, field, show) {
    var input = root.querySelector('input[data-field="' + field + '"]');
    if (input) input.type = show ? 'text' : 'password';
    var btn = root.querySelector('button[data-action="toggle-pw"][data-target="' + field + '"]');
    if (btn) {
  ...
  PasswordInput.EyeIcon = EyeIcon;
  PasswordInput.EyeOffIcon = EyeOffIcon;

  global.XL = global.XL || {};
  global.XL.PasswordInput = PasswordInput;
})(window);
